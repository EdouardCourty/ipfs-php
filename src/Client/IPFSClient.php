...
use IPFS\Transformer\FolderTransformer;
...
class IPFSClient
{
    ...
    public function addFolder(string $path, array $parameters = []): Folder
    {
        ...
        $folderTransformer = new FolderTransformer();

        $folderData = array_shift($deserializedParts);
        $folder = $folderTransformer->transform($folderData);

        $fileTransformer = new FileTransformer();
        foreach ($deserializedParts as $part) {
            $folder->addFile($fileTransformer->transform($part));
        }
